...
public class DerbyAccess extends DatabaseAccess {
     ...
     */
    private void updateIdentityStarts(ExtendedJdbcTemplate ejt) {
// DHOU
//log.warn("updateIdentityStarts 1");
...
// DHOU
//log.warn(is.table+" "+is.column+" "+is.aiValue);
log.warn("updateIdentityStarts 2.1");
            int maxId = ejt.queryForInt("select max(" + is.column + ") from " + is.table);
// DHOU
//log.warn("updateIdentityStarts 2.2");
            if (is.aiValue <= maxId)
                ejt.execute("alter table " + is.table + " alter column " + is.column + " restart with " + (maxId + 1));
// DHOU
//log.warn("updateIdentityStarts 2.3");
        }
    }
